...
export class CatalogService {
  ...
  async create(createCatalogDto: CreateCatalogDto) {
    try {
      const { catalogName } = createCatalogDto;
      ...
      const data = this.catalogRepo.create({ catalogName });
  ...
  async update(id: number, updateCatalogDto: UpdateCatalogDto) {
    try {
      const { catalogName } = updateCatalogDto;
      await this.catalogRepo.update({ id }, { catalogName });
